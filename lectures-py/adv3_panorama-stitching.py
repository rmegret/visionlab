...
# 
# Enhanced from the original demo as featured in [the scikit-image
# paper](https://peerj.com/articles/453/).
# 
# Multiple overlapping images of the same scene, combined into a single image,
# can yield amazing results. This tutorial will illustrate how to accomplish
# panorama stitching using scikit-image, from loading the images to cleverly
...
# ### Load data
# 
# The ``ImageCollection`` class provides an easy and efficient way to load and
# represent multiple images. Images in the ``ImageCollection`` are not only read
# from disk when accessed.
# 
# Load a series of images into an ``ImageCollection`` with a wildcard, as they
# share similar names.
...
# <markdowncell>
# Credit: Images of Private Arch and the trail to Delicate Arch in Arches
# National Park, USA, taken by Joshua D. Warner.<br>
# License: CC-BY 4.0
# 
# ---
...
# * Conversion to grayscale, as some feature descriptors are not defined for
# color images
# * Cropping to region(s) of interest
# 
# For convenience our example data is already resized smaller, and we won't
# bother cropping. However, they are presently in color so coversion to
...
# 
# We need to estimate a projective transformation that relates these images
# together. The steps will be
# 
# 1. Define one image as a _target_ or _destination_ image, which will remain
# anchored while the others are warped
# 2. Detect features in all three images
# 3. Match features from left and right images against the features in the
# center, anchored image.
# 
# In this three-shot series, the middle image `pano1` is the logical anchor
# point.
# 
# We detect "Oriented FAST and rotated BRIEF" (ORB) features in both images.
# 
# **Note:** For efficiency, in this tutorial we're finding 800 keypoints. The
# results are good but small variations are expected. If you need a more robust
# estimate in practice, run multiple times and pick the best result _or_
...
# 
# To filter out the false matches, we apply RANdom SAmple Consensus (RANSAC), a
# powerful method of rejecting outliers available in
# ``skimage.transform.ransac``.  The transformation is estimated using an
# iterative process based on randomly chosen subsets, finally selecting the
# model which corresponds best with the majority of matches.
# 
# We need to do this twice, once each for the transforms left -> center and
# right -> center.
...
# <markdowncell>
# The `inliers` returned from RANSAC select the best subset of matches. How do
# they look?
...
# # 3. Warping
# 
# Next, we produce the panorama itself. We must _warp_, or transform, two of the
# three images so they will properly align with the stationary image.
# 
...
# 
# Warp the images with `skimage.transform.warp` according to the estimated
# models. A shift, or _translation_ is needed to place as our middle image in
# the middle - it isn't truly stationary.
# 
# Values outside the input images are initially set to -1 to distinguish the
# "background", which is identified for later use.
# 
...
# <markdowncell>
# **What happened?!** Why are there nasty dark lines at boundaries, and why does
# the middle look so blurry?
# 
# 
# The **lines are artifacts (boundary effect) from the warping method**. When
# the image is warped with interpolation, edge pixels containing part image and
# part background combine these values. We would have bright lines if we'd
# chosen `cval=2` in the `warp` calls (try it!), but regardless of choice there
# will always be discontinuities.
# 
# ...Unless you use `order=0` in `warp`, which is nearest neighbor. Then edges
# are perfect (try it!). But who wants to be limited to an inferior
...
# <markdowncell>
# The surrounding flat gray is zero. _A perfect overlap would show no
# structure!_
# 
# Instead, the overlap region matches fairly well in the middle... but off to
# the sides where things start to look a little embossed, a simple average blurs
# the result. This caused the blurring in the previous, method (look again).
# _Unfortunately, this is almost always the case for panoramas!_
# 
# How can we fix this?
# 
# Let's attempt to find a vertical path through this difference image which
# stays as close to zero as possible. If we use that to build a mask, defining a
# transition between images, the result should appear _seamless_.
# 
# ---
...
# 
# 
# The array is called a _cost array_, while the path found is a _minimum-cost
# path_ or **MCP**.
# 
...
# 
# This method is so powerful that, with a carefully constructed cost array, the
# seed points are essentially irrelevant. It just works!
...
# 
# This utility function exists to give a "cost break" for paths from the edge to
# the overlap region.
# 
...
# <markdowncell>
# The tweak we made with `generate_costs` is subtle but important. Can you see
# it?
...
# 
# Use `skimage.graph.route_through_array` to find an optimal path through the
# cost array
...
# <markdowncell>
# That looks like a great seam to stitch these images together - the path looks
# very close to zero.
...
# ### Irregularities
# 
# Due to the random element in the RANSAC transform estimation, everyone will
# have a slightly different blue path. **Your path will look different from
# mine, and different from your neighbor's.** That's expected! _The awesome
# thing about MCP is that everyone just calculated the best possible path to
# stitch together their unique transforms!_


# <markdowncell>
# ### Filling the mask
# 
# Turn that path into a mask, which will be 1 where we want the left image to
# show through and zero elsewhere. We need to fill the left side of the mask
# with ones over to our path.
# 
# **Note**: This is the inverse of NumPy masked array conventions
# (``numpy.ma``), which specify a negative mask (mask == bad/missing) rather
# than a positive mask as used here (mask == good/selected).
# 
...
# <markdowncell>
# Label the various contiguous regions in the image using
...
# <markdowncell>
# **Add an additional constraint this time**, to prevent this path crossing the
# prior one!
...
# ### Final mask
# 
# The last mask for the middle image is one of exclusion - it will be displayed
...
# <markdowncell>
# What we have here is the world's most complicated and precisely-fitting jigsaw
# puzzle...
# 
...
# <markdowncell>
# Fantastic! Without the black borders, you'd never know this was composed of
# separate images!
# 
# ---
...
# 
# We converted to grayscale for ORB feature detection, back in the initial
# **preprocessing** steps. Since we stored our transforms and masks, adding
...
# 
# I hear what you're saying. "But Josh, those were too easy! The panoramas had
# too much overlap! Does this still work in the real world?"


# <markdowncell>
# **Go back to the top. Under "Load Data" replace the string `'data/JDW_03*'`
# with `'data/JDW_9*'`, and re-run all of the cells in order.**
